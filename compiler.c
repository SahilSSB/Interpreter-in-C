...
static void emitLoop(int loopStart) {
  ...
  if (offset > UINT16_MAX) error("Loop body too latge.");
...
static void binary(bool canAssign) {
  ...
  switch (operatorType) {
    ...
    case TOKEN_GREATER:       emitByte(OP_GREATER);
                              break;
    case TOKEN_GREATER_EQUAL: emitBytes(OP_LESSER, OP_NOT);
                              break;
    case TOKEN_LESSER:        emitByte(OP_LESSER);
                              break;
    case TOKEN_LESSER_EQUAL:  emitBytes(OP_GREATER, OP_NOT);
                              break;
...
static void forStatement() {
  ...
  if (!match(TOKEN_SEMICOLON)) {
    ...
    //jump out of th eloop if the condition is false.
...
static void grouping(bool canAssign) {
  expression();
  consume(TOKEN_RIGHT_PAREN, "Expext ')' after expression.");
}
...
ParseRule rules[] = {
  ...
  [TOKEN_BANG_EQUAL]    = {NULL,     binary,   PREC_EQUALILTY},
  [TOKEN_EQUAL]         = {NULL,     NULL,   PREC_NONE},
  [TOKEN_EQUAL_EQUAL]   = {NULL,     binary,   PREC_EQUALILTY},
